...
export function Sidebar({ isOpen, onClose }: SidebarProps) {
  ...
  return (
    <>
      ...
      <aside className={cn(
        "w-[280px] h-screen bg-sidebar text-white flex flex-col fixed left-0 top-0 overflow-y-auto z-50 transition-transform duration-300 lg:translate-x-0",
        ...
        <div className="p-6 flex flex-col gap-1 border-b border-sidebar-border border-opacity-20 relative">
          ...
          {/* Language Toggle */}
          <div className="absolute top-6 right-6 hidden sm:flex items-center gap-2 text-xs font-medium text-slate-400">
        ...
        <div className="p-4 mt-auto border-t border-slate-800">
          ...
          <button className="flex items-center gap-3 px-4 py-3 mt-2 w-full rounded-lg text-sm font-medium text-slate-400 hover:text-white hover:bg-slate-800/30 transition-colors">
            <LogOut size={18} />
            Logout
          </button>
        </div>
